    ...
    for (int i = 0; i < natoms; i++){
        ...
        for (int  k1 = 0; k1 < Grid[0]; k1++){
            ...
            for (int  n1 = -l_max; n1 < l_max+1; n1++){
                x_direc[i][k1]+=M_n(u[i][0]-k1-n1*K,n[0]);
            }
        }
        // for Y direction
        for (int  k2 = 0; k2 < Grid[1]; k2++){
            ...
            for (int  n2 = -l_max; n2 < l_max+1; n2++){
                y_direc[i][k2]+=M_n(u[i][1]-k2-n2*K,n[1]);
            }
        }
        // for Z direction
    ...
    for (int i = -K; i < K+1; i++){
        for (int j = -K; j< K+1; j++){
            for (int k = -K; k < K+1; k++){
                ...
                reciprocal_energy_i+= norm_FQ * norm(Coeff(TwoPi_Grid[0]*i,n[0])*Coeff(TwoPi_Grid[1]*j,n[1])*Coeff(TwoPi_Grid[2]*k,n[2])) / (factor*exp(pi_sq*factor/beta_sq));
            }
        }
    }
    reciprocal_energy_i*=1/(2*M_PI*Length[0]*Length[1]*Grid[2]);
